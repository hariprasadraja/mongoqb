...
var elementalOperators map[string]string
...
func init() {
	...
	elementalOperators = make(map[string]string, 4)
	elementalOperators["(ex)"] = "$exists"
	elementalOperators["(ty)"] = "$type"

}

//Config provides facility to configure parser to parse requset url.
type Config struct {

	// RootKey is a name of the key in http request url params.
	// By default it is "filterBy".
	// It defines what are the keys avalialble in the request and it's operatations.
	// example: http://localhost:9000/?filterBy=name,id - here filterBy is the rootkey.
	RootKey string

	// DelimiterL contains string which sepeate the operator from the left.
	// Default DelimiterL is (
	DelimiterLeft string

	// DelimiterR contains string which sepeate the operator from the right.
	// Default DelimiterR is (
	DelimiterR string
}
